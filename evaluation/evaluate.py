...
def evaluate_retrieval(questions: list) -> dict:
    ...
    for q in retrieval_questions:
        ...
        # FAISS 粗排 30 条 → Rerank 精排取 5 条（与线上配置一致）
        docs_30 = faiss_db.similarity_search(q["question"], k=30)
        try:
            docs_top5 = _rerank(q["question"], docs_30, top_k=5)
...
def evaluate_tool_selection(questions: list, rounds: int = 1) -> dict:
    ...
    for q in questions:
        ...
        if q != questions[-1]:  # 最后一题不用等
            ...
            if extra:
                msg += f"（检测到 RateLimit，额外+{extra}s）"
...
if __name__ == "__main__":
    ...
    if args.retrieval_only:
        ...
        questions = json.loads(QUESTIONS_PATH.read_text(encoding="utf-8"))
        if not check_server():
            print("\n⚠️  服务器未启动！请先运行：python main.py")
            sys.exit(1)
        tool_report = evaluate_tool_selection(questions, args.rounds)
        RESULTS_PATH.write_text(
            json.dumps({"timestamp": datetime.now().isoformat(), "tool_selection": tool_report},
                       ensure_ascii=False, indent=2),
            encoding="utf-8",
        )
        print(f"\n结果已保存至: {RESULTS_PATH}")
